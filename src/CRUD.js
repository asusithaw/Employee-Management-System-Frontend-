...
const CRUD = () => {
   ...
   useEffect(()=>{        
    ...
    const getData = () =>{
      axios.get('http://localhost:5016/api/Employee/GetEmployees')
      .then((result)=>{
        setData(result.data)
      })
      .catch((error) =>{
    ...
    const handleEdit =(id) => {        
        handleShow();
        axios.get(`http://localhost:5016/api/Employee/${id}`)
        .then((result) => {
    ...
    const handleDelete =(id) => {
        if(window.confirm("Are you sure to Delete this Employee") == true){
          axios.delete(`http://localhost:5016/api/Employee/${id}`)
            .then((result) => {
    ...
    const handleUpdate = () => {
      const url = `http://localhost:5016/api/Employee?id=${editID}`;
      const data = {
          id: editID,
          name: editName,
          age: editAge,
    ...
    const handleSave =() => {
      const url = 'http://localhost:5016/api/Employee/CreateEmployee';
      const data = {
        "name": name,
        "age": age,
        "isActive": isActive
